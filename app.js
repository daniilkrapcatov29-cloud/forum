(function () {
	...
	const RANKS = {
		"РУКОВОДСТВО ПРОЕКТА":     { color: "#ff3838", icon: "👑", priority: 100 },
		"РУКОВОДИТЕЛЬ АДМИНИСТРАЦИИ": { color: "#ff6b1a", icon: "🔶", priority: 90 },
		"АДМИНИСТРАТОР":           { color: "#ff9500", icon: "🛡️", priority: 80 },
		"КОМАНДА ПРОЕКТА":         { color: "#9b6cff", icon: "💎", priority: 70 },
		"МОДЕРАТОР":               { color: "#00d4a8", icon: "🔧", priority: 60 },
		"ИГРОК":                   { color: "#9aa0b4", icon: "🎮", priority: 0 },
	};
	const RANK_KEYS = Object.keys(RANKS);
	const ADMIN_RANKS = ["РУКОВОДСТВО ПРОЕКТА", "РУКОВОДИТЕЛЬ АДМИНИСТРАЦИИ", "АДМИНИСТРАТОР"];
	...
	const DEFAULT_DATA = {
		users: [
			{ id: 1, name: "Гоша", role: "РУКОВОДСТВО ПРОЕКТА", color: "#ff3838", joined: Date.now() - 2592e6, posts: 42 },
			{ id: 2, name: "Алексей", role: "РУКОВОДИТЕЛЬ АДМИНИСТРАЦИИ", color: "#ff6b1a", joined: Date.now() - 216e7, posts: 28 },
			{ id: 3, name: "Максим", role: "МОДЕРАТОР", color: "#00d4a8", joined: Date.now() - 1728e6, posts: 15 },
			{ id: 4, name: "Дарья", role: "КОМАНДА ПРОЕКТА", color: "#9b6cff", joined: Date.now() - 1296e6, posts: 31 },
		],
		categories: [
			{
	...
	function ensureGoshaAdmin() {
		...
		if (!gosha) {
			gosha = { id: Date.now(), name: "Гоша", role: "РУКОВОДСТВО ПРОЕКТА", color: "#ff3838", joined: Date.now(), posts: 0 };
	...
	function renderUserActions() {
		...
		if (session) {
			el.innerHTML =
				...
				'<span class="rank-badge" style="--rc:' + rankColor(session.role) + '">' + rankInfo(session.role).icon + " " + escapeHtml(session.role) + "</span>" +
	...
	function viewForum(fid) {
		...
		render(
			...
			'<div class="page-head"><div class="page-title"><h1>' + forum.icon + " " + escapeHtml(forum.name) + '</h1></div>' + newBtn + "</div>" +
	...
	function viewAuth(mode) {
		...
		$("#authSubmit").onclick = () => {
			...
			if (isLogin) {
				...
				const color = isGosha(name) ? "#ff3838" : avatarColor(name);
	...
	function viewMembers() {
		...
		render(
			...
			'<div class="thread-list">' +
				sorted.map((u) =>
					'<div class="thread-row members-row">' +
		...
		render(
			'<div class="admin-wrap">' +
				'<div class="admin-header"><h1>⚡ Админ-панель</h1>' +
					(session && isGosha(session.name) ? '<span class="rank-badge" style="--rc:#ff3838;font-size:12px">👑 Вы вошли как Гоша</span>' : "") +
					'<button class="btn btn-danger" id="adminLogout">Выйти</button></div>' +
				adminNav +
	...
	function adminUsers() {
		...
		return '<div class="admin-card"><h3>Пользователи (' + data.users.length + ")</h3>" +
			'<div class="rank-info-box">💡 Выдавайте ранги пользователям через выпадающий список. Ранг отображается около ника на форуме. Ранг Гоша защищён от изменения.</div>' +
		...
		document.querySelectorAll("[data-role-user]").forEach((sel) => {
			sel.onchange = () => {
				...
				if (u) {
					...
					sel.parentElement.querySelector(".rank-badge")?.replaceWith(document.createRange().createContextualFragment(rankBadge(u.role)));
				}
			};
		});
	}
