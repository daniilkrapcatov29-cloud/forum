(function () {
	...
	const DEFAULT_DATA = {
		users: [
			{ id: 1, name: "Администратор", role: "Администратор", color: "#ff5c7c", joined: Date.now() - 9e8, posts: 1280 },
			{ id: 2, name: "Модератор форума", role: "Модератор", color: "#6c8cff", joined: Date.now() - 7e8, posts: 642 },
			{ id: 3, name: "Дмитрий К.", role: "Игрок", color: "#00d4a8", joined: Date.now() - 3e8, posts: 87 },
			{ id: 4, name: "Анна Смирнова", role: "Игрок", color: "#ffb84d", joined: Date.now() - 2e8, posts: 34 },
		],
		categories: [
			{
	...
	function getParentForum(subId) {
		for (const cat of data.categories) {
			for (const f of cat.forums) {
				if ((f.subforums || []).some((s) => s.id === subId)) return f;
			}
		}
		return undefined;
	}
			...
			<section class="category">
				...
				<div class="forum-row" data-go="#/forum/${f.id}">
					...
					<div class="forum-meta">
						...
						${subs.length ? `<div class="forum-subs">📁 ${subs.map((s) => `<a href="#/forum/${s.id}" onclick="event.stopPropagation()">${escapeHtml(s.name)}</a>`).join(" · ")}</div>` : ""}
	...
	function viewForum(fid) {
		...
		const subBox = subs.length ? `
			<div class="subforum-list">
				...
				${subs.map((s) => `
					<div class="forum-row subforum-row" data-go="#/forum/${s.id}">
						<div class="forum-icon-sm">${s.icon || "📁"}</div>
						<div class="forum-meta">
							<h3>${escapeHtml(s.name)}</h3>
							<p>${escapeHtml(s.desc || "Без описания")}</p>
						</div>
						<div class="forum-stats muted-sm">${fmt(s.threads || 0)} тем</div>
					</div>`).join("")}
			</div>` : "";
		...
		const crumb = parent ? `<a href="#/">Главная</a><span>›</span><a href="#/forum/${parent.id}">${escapeHtml(parent.name)}</a><span>›</span><span>${escapeHtml(forum.name)}</span>` : `<a href="#/">Главная</a><span>›</span><span>${escapeHtml(forum.name)}</span>`;
		render(`
			...
			<div class="admin-card">
				...
				<div class="admin-table">
					${cat.forums.map((f) => {
						const subs = f.subforums || [];
						return `
						<div class="admin-row admin-forum-row">
						...
						${subs.map((s) => `
							<div class="admin-row admin-sub-row">
								<span class="forum-icon-sm sub-indent">${s.icon || "📁"}</span>
								<div><span class="sub-badge">Подраздел</span> ${escapeHtml(s.name)}<br><span class="muted">${escapeHtml(s.desc || "Без описания")}</span></div>
								<div class="muted">${fmt(s.threads || 0)} тем</div>
								<div class="admin-row-btns">
									<button class="btn btn-danger btn-sm" data-del-sub="${f.id}|${s.id}">Удалить</button>
								</div>
							</div>`).join("")}
						<div class="admin-sub-form" id="subform-${f.id}" hidden>
							<input type="text" class="sub-input-icon" placeholder="📁" maxlength="4">
							<input type="text" class="sub-input-name" placeholder="Название подраздела">
							<input type="text" class="sub-input-desc" placeholder="Описание">
							<button class="btn btn-primary btn-sm" data-create-sub="${f.id}">Создать</button>
							<button class="btn btn-ghost btn-sm" data-cancel-sub="${f.id}">Отмена</button>
						</div>`;
					}).join("")}
				</div>
			</div>`).join("");
	...
	function bindAdminActions() {
		document.querySelectorAll("[data-del-forum]").forEach((b) => {
			b.onclick = () => {
				...
				if (!confirm("Удалить раздел? Все подразделы и темы будут удалены.")) return;
		...
		document.querySelectorAll("[data-add-sub]").forEach((b) => {
			b.onclick = () => {
				const fid = b.dataset.addSub;
				document.querySelectorAll(".admin-sub-form").forEach((f) => f.hidden = true);
				const form = document.getElementById("subform-" + fid);
				if (form) { form.hidden = false; form.querySelector(".sub-input-name").focus(); }
			};
		});
		document.querySelectorAll("[data-cancel-sub]").forEach((b) => {
			b.onclick = () => { document.getElementById("subform-" + b.dataset.cancelSub).hidden = true; };
		});
		document.querySelectorAll("[data-create-sub]").forEach((b) => {
			b.onclick = () => {
				const fid = b.dataset.createSub;
				const form = document.getElementById("subform-" + fid);
				const icon = form.querySelector(".sub-input-icon").value.trim() || "📁";
				const name = form.querySelector(".sub-input-name").value.trim();
				const desc = form.querySelector(".sub-input-desc").value.trim();
				if (!name) return alert("Введите название подраздела");
				const parent = getForum(fid);
				if (!parent) return;
				if (!parent.subforums) parent.subforums = [];
				parent.subforums.push({ id: "s" + Date.now(), icon, name, desc: desc || "Без описания", threads: 0, posts: 0 });
				saveData(data);
				viewAdmin("forums");
			};
		});
		document.querySelectorAll("[data-del-sub]").forEach((b) => {
			b.onclick = () => {
				...
				const parent = getForum(fid);
				if (parent && parent.subforums) parent.subforums = parent.subforums.filter((s) => s.id !== sid);
