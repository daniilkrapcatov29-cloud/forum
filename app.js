			...
			'<div class="admin-wrap">' +
				'<div class="admin-header"><h1>⚡ Админ-панель</h1>' +
					'<div class="admin-header-right">' +
						(session && isGosha(session.name) ? '<span class="rank-badge" style="--rc:#ff3838;font-size:12px">👑 Гоша</span>' : "") +
	...
	function bindAdminActions() {
		...
		document.querySelectorAll("[data-del-user]").forEach((b) => {
			b.onclick = () => {
				...
				const u = getUser(uid);
				if (!u) return;
				if (!confirm('Удалить пользователя "' + u.name + '"? Все его темы останутся, но будут без автора.')) return;
